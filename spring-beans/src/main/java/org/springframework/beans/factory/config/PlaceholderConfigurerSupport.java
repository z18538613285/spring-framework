...
public abstract class PlaceholderConfigurerSupport extends PropertyResourceConfigurer
	 ...
	 * 流程如下：
	 *
	 * （1）根据 String 值解析策略 valueResolver 得到 BeanDefinitionVisitor 实例。BeanDefinitionVisitor 是 BeanDefinition 的访问者，我
	 * 们通过它可以实现对 BeanDefinition 内容的进行访问，内容很多，例如Scope、PropertyValues、FactoryMethodName 等等。
	 * （2）得到该容器的所有 BeanName，然后对其进行访问（visitBeanDefinition()）。
	 * （3）解析别名的占位符
	 * （4）解析嵌入值的占位符，例如注释属性
	 * 这个方法核心在于 visitBeanDefinition() 的调用，
