 ...
 *
 * @tips 基于内容类型来获取对应 View 的 ViewResolver 实现类。
...
public class ContentNegotiatingViewResolver extends WebApplicationObjectSupport
	 ...
	 * ViewResolver 数组
	 *
	 * ContentNegotiatingViewResolver 会使用这些 viewResolvers 们，解析出所有的 View 们，
	 * 然后基于内容类型来获取对应的 View 们。此时的 View 结果，可能是一个，可能是多个，
	 * 所以需要比较获取到最优的 View 对象。
	 */
	@Nullable
		...
		if (this.viewResolvers == null) {
			...
			for (ViewResolver viewResolver : matchingBeans) {
				if (this != viewResolver) { // 排除自己
					/**
					 * 默认情况下，走的是这段逻辑。所以此时 viewResolvers 会有
					 * BeanNameViewResolver、ThymeleafViewResolver、ViewResolverComposite、InternalResourceViewResolver 四个对象。
					 */
	...
	private View getBestView(List<View> candidateViews, List<MediaType> requestedMediaTypes, RequestAttributes attrs) {
		...
		for (MediaType mediaType : requestedMediaTypes) {
			...
			// 也就是说，优先级的匹配规则，由 ViewResolver 在 viewResolvers 的位置，越靠前，优先级越高。
			for (View candidateView : candidateViews) {
